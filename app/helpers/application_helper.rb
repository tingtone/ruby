module ApplicationHelper
  ...
  def topic_title_link(topic, options)
    if topic.title =~ /^\[([^\]]{1,15})\]((\s+)\w+.*)/
      ...
      link_to(h($2.strip), forum_forum_topic_path(@forum, topic), options)
    else
      link_to(h(topic.title), forum_forum_topic_path(@forum, topic), options)
    end
  end
  ...
  def moderator_of?(record)
      ...
      current_parent.moderator_of? forum
  end
